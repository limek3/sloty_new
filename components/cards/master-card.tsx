          ...
          <div className="flex items-end justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="truncate ty-body font-bold leading-tight text-white">
                    {name}
                  </div>
                  <div className="truncate text-sm text-white/80">{subtitle}</div>
                </div>
...
function CompactCard({ master, onClick }: { master: any; onClick?: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <div className="truncate ty-body font-bold text-foreground">{name}</div>
        <div className="truncate ty-subtitle text-muted-foreground">{subtitle}</div>
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <div className="truncate ty-body font-bold leading-tight text-foreground">{name}</div>
                {verified && <BadgeCheck className="h-4 w-4 shrink-0 text-success" />}
              </div>
              <div className="mt-0.5 truncate ty-body text-muted-foreground">{subtitle}</div>
            </div>
          ...
          <div className="mt-2.5 flex flex-wrap items-center gap-2 ty-body">
      ...
      <div className="mt-3 flex items-center justify-between gap-2 rounded-[18px] bg-secondary/55 px-3 py-2.5">
        <div className="min-w-0 flex flex-wrap items-center gap-1.5">
          {tags.length > 0 ? tags.map((tag, index) => (
            <span
              key={`${tag}-${index}`}
              className="inline-flex items-center rounded-full border border-border bg-white px-2.5 py-1 ty-subtitle font-medium text-secondary-foreground"
            >
              {tag}
            </span>
          )) : (
            <span className="ty-body text-muted-foreground">Свободные слоты сегодня</span>
          )}
        </div>

        <div className="flex shrink-0 items-center gap-2">
          <div className="rounded-full bg-emerald-50 px-3 py-1.5 ty-body font-bold text-success">{price}</div>
