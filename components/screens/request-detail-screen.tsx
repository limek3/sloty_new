    ...
    <div className="min-h-screen bg-[#f6f6f3] pb-24 safe-top safe-bottom">
      ...
      <main className="mx-auto max-w-2xl space-y-2.5 px-3 py-3">
        <section className="rounded-[18px] border border-black/6 bg-white p-2.5 shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
          <span className="inline-flex rounded-full border border-black/6 bg-[#fafaf8] px-2 py-0.5 text-[8px] font-medium text-slate-700">
            {request.category}
          </span>

          <h2 className="mt-2 text-[12px] font-bold leading-[1.4] tracking-tight text-slate-900">
            {request.title}
          </h2>
        ...
        <section className="rounded-[18px] border border-black/6 bg-white p-2.5 shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-[10px] font-bold text-emerald-700">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[8px] text-slate-500">{request.createdAt}</p>
            </div>
        ...
        <section className="grid grid-cols-2 gap-2">
          <div className="rounded-[14px] border border-black/6 bg-white p-2.5 shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
            <p className="text-[8px] text-slate-500">{t('budget')}</p>
            <p className="mt-1 text-[13px] font-bold tracking-tight text-emerald-600">
              {formatPrice(request.budget)}
            </p>
          </div>

          <div className="rounded-[14px] border border-black/6 bg-white p-2.5 shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
            <p className="text-[8px] text-slate-500">{t('responses')}</p>
            <p className="mt-1 text-[13px] font-bold tracking-tight text-slate-900">
        ...
        <section className="rounded-[18px] border border-black/6 bg-white p-2.5 shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
          <div className="space-y-2">
            <div className="flex items-center gap-2.5 rounded-[12px] bg-[#f7f7f5] px-2.5 py-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-white text-slate-500">
                <MapPin className="h-3 w-3" />
              </div>
              <div>
                <p className="text-[7px] text-slate-400">{t('location')}</p>
                <p className="text-[9px] font-medium text-slate-800">{request.location}</p>
              </div>
            </div>
        ...
        {userRole === 'master' && request.responsesCount > 0 && (
          <section className="rounded-[18px] border border-black/6 bg-white p-2.5 shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
            ...
            <p className="mt-1 text-[9px] leading-[1.4] text-slate-500">
              {isRu
      ...
      {userRole === 'master' && request.status === 'open' && (
        <div className="fixed inset-x-0 bottom-0 z-40 px-3 pb-3 safe-bottom">
          <div className="rounded-[18px] border border-black/6 bg-white p-2 shadow-[0_8px_26px_rgba(15,23,42,0.08)]">
            <Button
              className="h-10 w-full gap-1.5 rounded-[12px] bg-emerald-500 text-[10px] font-semibold text-white shadow-[0_12px_26px_rgba(16,185,129,0.2)] hover:bg-emerald-600"
              onClick={() => {
                navigate('chats');
              }}
            >
