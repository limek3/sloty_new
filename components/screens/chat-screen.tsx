    ...
    return (
      <div className="min-h-screen bg-[#f6f6f3] px-4 safe-top safe-bottom">
        <div className="flex min-h-screen items-center justify-center">
          <div className="rounded-[16px] border border-black/6 bg-white px-4 py-6 text-center shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
    ...
    <div className="fixed inset-0 flex flex-col bg-[#f6f6f3] safe-top safe-bottom">
      <header className="shrink-0 px-3 pt-3">
        <div className="mx-auto max-w-2xl">
          <div className="rounded-[22px] border border-black/5 bg-white p-2.5 shadow-[0_8px_28px_rgba(15,23,42,0.05)]">
            <div className="flex items-start gap-2.5 px-0.5 pb-1 pt-0.5">
              ...
              <div className="flex min-w-0 flex-1 items-center gap-2.5">
                {chat.participantAvatar ? (
                  ...
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[12px] bg-emerald-100 text-[10px] font-bold text-emerald-700">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1">
                    <h2 className="truncate text-[11px] font-bold tracking-tight text-slate-900">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                <button
                  className="flex h-9 w-9 items-center justify-center rounded-[12px] border border-black/6 bg-[#f7f7f5] text-slate-500 transition hover:bg-white hover:text-emerald-600"
                ...
                <button
                  className="flex h-9 w-9 items-center justify-center rounded-[12px] border border-black/6 bg-[#f7f7f5] text-slate-500 transition hover:bg-white"
            ...
            <div className="mt-1 flex items-center justify-between rounded-[10px] border border-black/6 bg-[#fafaf8] px-2.5 py-1.5">
              <div className="flex items-center gap-1 text-[8px] text-slate-600">
                <Sparkles className="h-3 w-3 text-emerald-600" />
                <span>{isRu ? 'Чат активен' : 'Chat is active'}</span>
              </div>

              <div className="inline-flex items-center gap-0.5 rounded-full bg-emerald-50 px-1.5 py-0.5 text-[8px] font-medium text-emerald-700">
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col overflow-hidden px-3 pb-3 pt-2.5">
        <div className="mb-2 flex items-center justify-center">
          <div className="rounded-full border border-black/6 bg-white px-2 py-0.5 text-[8px] font-medium text-slate-500 shadow-[0_4px_12px_rgba(15,23,42,0.03)]">
            {isRu ? 'Сегодня' : 'Today'}
          </div>
        </div>
                  ...
                  <div
                    className={`rounded-[14px] px-2.5 py-2 shadow-[0_4px_16px_rgba(15,23,42,0.03)] ${
                      msg.isOwn
                        ...
                        : 'rounded-bl-[6px] border border-black/6 bg-white text-slate-800'
                    }`}
                  >
                    <p className="text-[10px] leading-[1.5]">{msg.text}</p>
        ...
        <div className="mt-2">
          <div className="mb-2 flex gap-1.5 overflow-x-auto pb-0.5 scrollbar-hide">
            {quickReplies.map((reply) => (
              <button
                ...
                className="shrink-0 rounded-full border border-black/6 bg-white px-2 py-1 text-[8px] font-medium text-slate-600 transition hover:bg-emerald-50 hover:text-emerald-700"
              >
                {reply}
              </button>
            ))}
          ...
          <div className="rounded-[18px] border border-black/6 bg-white p-2 shadow-[0_8px_26px_rgba(15,23,42,0.06)]">
            <div className="flex items-end gap-1.5">
              <button
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[12px] border border-black/6 bg-[#f7f7f5] text-slate-500 transition hover:bg-white"
              ...
              <div className="relative min-w-0 flex-1">
                <textarea
                  ...
                  rows={1}
                  className="max-h-24 min-h-[36px] w-full resize-none rounded-[12px] border border-black/6 bg-[#fafaf8] px-2.5 py-2 pr-9 text-[9px] text-slate-900 outline-none placeholder:text-slate-400 focus:border-emerald-200 focus:bg-white"
                />

                <button
                  className="absolute bottom-1.5 right-1.5 flex h-5 w-5 items-center justify-center rounded-full text-slate-400 transition hover:bg-white hover:text-emerald-600"
