          ...
          <div className="rounded-[22px] border border-border/70 bg-card p-3 shadow-[0_8px_28px_rgba(15,23,42,0.05)]">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 ty-body text-slate-500">
                  {master.specialization}
                </p>

                {/* Stats */}
                <div className="mt-2 flex flex-wrap items-center gap-2 ty-subtitle">
              ...
              <div className="shrink-0 rounded-[14px] bg-emerald-50 px-2.5 py-2 text-right">
                <span className="ty-body text-slate-500">{t('from')}</span>
                <p className="ty-subtitle font-bold text-emerald-600">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <Button
                variant="outline"
                className="h-10 rounded-[14px] gap-1.5 border-border/70 bg-[#f7f7f5] ty-body shadow-none hover:bg-card"
                onClick={handleChat}
              >
            ...
            <div className="mt-3 rounded-[14px] border border-border/70 bg-[#fafaf8] p-2.5">
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate ty-body font-medium text-slate-900">
                    {master.address}
                  </p>
                  <p className="mt-0.5 ty-body text-slate-500">
                ...
                <button
                  onClick={handleBuildRoute}
                  className="shrink-0 rounded-[10px] bg-emerald-50 px-2.5 py-1.5 ty-subtitle font-semibold text-emerald-700 transition hover:bg-emerald-100"
                >
                  {isRu ? 'Маршрут' : 'Route'}
                </button>
              </div>
        ...
        <div className="mx-auto max-w-2xl">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-4 h-10 rounded-[14px] bg-[#f0f0ec] p-1">
              <TabsTrigger value="services" className="rounded-[10px] ty-subtitle font-medium data-[state=active]:bg-card data-[state=active]:shadow-[0_4px_12px_rgba(15,23,42,0.05)]">
                {t('services')}
              </TabsTrigger>
              <TabsTrigger value="reviews" className="rounded-[10px] ty-subtitle font-medium data-[state=active]:bg-card data-[state=active]:shadow-[0_4px_12px_rgba(15,23,42,0.05)]">
                {t('reviews')}
              </TabsTrigger>
              <TabsTrigger value="portfolio" className="rounded-[10px] ty-subtitle font-medium data-[state=active]:bg-card data-[state=active]:shadow-[0_4px_12px_rgba(15,23,42,0.05)]">
                {t('portfolio')}
              </TabsTrigger>
              <TabsTrigger value="about" className="rounded-[10px] ty-subtitle font-medium data-[state=active]:bg-card data-[state=active]:shadow-[0_4px_12px_rgba(15,23,42,0.05)]">
                {t('about')}
              </TabsTrigger>
            </TabsList>
              ...
              {master.services.map((service) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2.5">
                    <div className="min-w-0 flex-1">
                      <h3 className="ty-body font-semibold text-slate-900 group-hover:text-emerald-600 transition-colors">
                        {service.name}
                      </h3>

                      {service.description && (
                        <p className="mt-0.5 line-clamp-2 ty-subtitle leading-[1.4] text-slate-500">
                          {service.description}
                        </p>
                      )}

                      <div className="mt-1.5 flex flex-wrap items-center gap-2">
                        <span className="ty-subtitle font-bold text-emerald-600">
                          {formatPrice(service.price)}
                        </span>

                        <span className="flex items-center gap-0.5 ty-subtitle text-slate-500">
                  ...
                  >
                    <div className="flex items-center justify-between gap-2.5 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 ty-subtitle font-bold text-slate-700">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="ty-body text-slate-400">{review.date}</p>
                        </div>
                      </div>
                    ...
                    <p className="ty-subtitle leading-[1.5] text-slate-700">{review.text}</p>
                  </div>
                ))
              ) : (
                <div className="text-center py-10">
                  <p className="ty-subtitle text-slate-500">
            ...
            <TabsContent value="about" className="mt-3 pb-32">
              <div className="rounded-[16px] border border-border/70 bg-card p-2.5 shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
                <h3 className="ty-body font-semibold text-slate-900 mb-1.5">{t('about')}</h3>
                <p className="ty-subtitle leading-[1.5] text-slate-700">{master.about}</p>
              </div>

              {master.tags.length > 0 && (
                <div className="mt-3">
                  <h3 className="ty-body font-semibold text-slate-900 mb-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {master.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full border border-border/70 bg-[#fafaf8] px-2.5 py-1 ty-subtitle text-slate-700"
                      >
                        {tag}
                      </span>
                    ))}
      ...
      <div className="fixed bottom-3 left-0 right-0 z-40 px-3 safe-bottom">
        <div className="mx-auto max-w-2xl">
          <div className="rounded-[18px] bg-card/95 p-2 shadow-[0_-8px_26px_rgba(15,23,42,0.08)] backdrop-blur-xl border border-border/70">
            <Button
              className="w-full h-11 ty-subtitle font-semibold rounded-[14px] bg-emerald-500 text-white shadow-[0_10px_24px_rgba(16,185,129,0.2)] hover:bg-emerald-600"
              onClick={() => handleBook()}
            >
