          ...
          <div className="rounded-[22px] border border-border/70 bg-card p-2.5 shadow-[0_8px_28px_rgba(15,23,42,0.05)]">
            <div className="flex items-center justify-between gap-2.5 px-0.5 pb-1.5 pt-0.5">
              ...
              <Button
                ...
                className="h-9 rounded-[12px] bg-emerald-500 px-2.5 ty-subtitle font-semibold text-white shadow-[0_10px_20px_rgba(16,185,129,0.2)] hover:bg-emerald-600"
              >
            ...
            <div className="mt-1 grid grid-cols-2 gap-1.5 rounded-[14px] border border-border/70 bg-[#fafaf8] p-1">
              <button
                ...
                className={`flex items-center justify-center gap-1 rounded-[10px] px-2.5 py-2 ty-subtitle font-medium transition-all ${
                  activeTab === 'browse'
      ...
      <main className="mx-auto max-w-2xl px-3 py-3">
        {displayRequests.length > 0 ? (
            ...
            {displayRequests.map((request) => (
              ...
              >
                <div className="flex items-start justify-between gap-2.5">
                  <h3 className="line-clamp-2 pr-2 ty-subtitle font-semibold leading-[1.4] text-slate-900 transition-colors group-hover:text-emerald-600">
                    {request.title}
                  </h3>
                ...
                <p className="mt-2 line-clamp-2 ty-subtitle leading-[1.5] text-slate-500">
                  {request.description}
                </p>

                <div className="mt-2 flex flex-wrap items-center gap-1">
                  <span className="rounded-full border border-border/70 bg-[#fafaf8] px-2 py-0.5 ty-body font-medium text-slate-700">
                    {request.category}
                  </span>

                  <div className="inline-flex items-center gap-0.5 rounded-full bg-slate-100 px-1.5 py-0.5 ty-body text-slate-600">
                ...
                <div className="mt-2.5 flex items-center justify-between border-t border-border/70 pt-2.5">
                  <div className="flex min-w-0 items-center gap-1.5">
                    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 ty-subtitle font-bold text-emerald-700">
                      {request.authorName.charAt(0)}
                    </div>

                    <div className="min-w-0">
                      <div className="truncate ty-subtitle font-medium text-slate-700">
                        {request.authorName}
                      </div>
                      <div className="ty-body text-slate-400">{request.createdAt}</div>
                    </div>
                  </div>

                  <div className="flex shrink-0 items-center gap-1.5">
                    <span className="ty-body font-bold tracking-tight text-emerald-600">
                    ...
                    <div className="inline-flex items-center gap-0.5 rounded-full border border-border/70 bg-[#fafaf8] px-1.5 py-0.5 text-slate-500">
                      ...
                      <span className="ty-body font-medium">{request.responsesCount}</span>
                    </div>
          ...
          <div className="rounded-[18px] border border-border/70 bg-card px-4 py-10 text-center shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
            ...
            <h3 className="mt-3 ty-body font-semibold text-slate-900">
              {activeTab === 'my'
                ? isRu
            ...
            <p className="mx-auto mt-1.5 max-w-xs ty-subtitle leading-[1.4] text-slate-500">
              {isRu
            ...
            <Button
              onClick={() => navigate('create-request')}
              className="mt-3 h-9 rounded-[12px] bg-emerald-500 px-3 ty-subtitle font-semibold text-white shadow-[0_10px_20px_rgba(16,185,129,0.2)] hover:bg-emerald-600"
            >
              <Plus className="mr-1 h-3.5 w-3.5" />
              {t('createRequest')}
            </Button>
