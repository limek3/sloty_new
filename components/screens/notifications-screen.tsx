      ...
      <main className="mx-auto max-w-2xl space-y-2.5 px-3 py-3">
        {items.map((item) => (
          <button
            key={item.id}
            onClick={item.action}
            className="group flex w-full items-start gap-2.5 rounded-[18px] border border-border/70 bg-card p-2.5 text-left shadow-elevation-card transition-all hover:-translate-y-[0.5px] hover:shadow-elevation-nav"
          >
            <div
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[7px] text-muted-foreground">{item.time}</p>
            </div>

            <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-surface-2 text-muted-foreground transition group-hover:bg-emerald-50 group-hover:text-emerald-600">
              <ChevronRight className="h-3 w-3" />
            </div>
          </button>
        ))}

        <div className="flex items-start gap-2.5 rounded-[14px] border border-border/70 bg-card p-2.5 shadow-elevation-card">
